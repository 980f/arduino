...
ChainPrinter dbg(debug); //expects a Stream named debug.  todo: less generic name for this, perhaps a #define with an #ifdef here to disable dbg messages
  ...
  unsigned Credentials::save(unsigned offset = 0) { //todo: symbols or static allocator.
    return strzsave(password, sizeof(password), strzsave(ssid, sizeof(ssid), offset));
  }

  unsigned Credentials::load(unsigned offset = 0) {
  ...
  void Credentials::setID(const char *s) {
    strzcpy(ssid, sizeof(ssid), s);
  }

  /**safe set of password, clips and ensures a trailing nul */
  void Credentials::setPWD(const char *s) {
    strzcpy(password, sizeof(password), s);
  }

  Telnetter::Telnetter(TelnetActor *actor, uint16_t teleport = 23): //23 is standard port
    teleport(teleport),
  ...
  */
  ...
  void Telnetter::broadcast(const uint8_t &sbuf, const unsigned len, const unsigned sender = ~0U) {
