...
class Flasher {
  ...
  MilliTick step[numSteps];

  unsigned int ledStep = 0;
  MilliTick stepDoneAt = 0;

  /** whether to show the number of times the LED has been toggled, with each toggle */
public:
  Flasher(unsigned pinNumber) : ledPin(pinNumber) {
    //do nothing on construction, can't even trust pinMode to work although all investigate architectures will do it correctly.
  }
...
public:
  ...
  void loop(MilliTick currentMillis) {
